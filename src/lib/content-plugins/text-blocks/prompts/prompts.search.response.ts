...
const handler: PromptHandlerFn = {
    create: (siteConfigs, siteData, input) => {
        ...
        intro += `In this case, your job is to generate a response based on provided paragraphs to complete the following task:\n`;
        ...
        switch (input.type) {
            case "Search Query":
                prompt += `${intro}\nThese paragraphs are search results to for the following keywords:\n`;
                prompt += input.concepts_key_terms.join(", ") + ".\n";
                prompt += "Generate a concise overview summary of this search based solely on the provided records. Do not use general knowledge or make assumptions:\n";
                break;

            case "Question":
                prompt += `${intro}\nAnswer the following question based on the provided records:\n`;
            ...
            case "Instruction":
                prompt += `${intro}\nFollow this instruction using the provided paragraphs:\n`;
            ...
            case "Complex Task":
                prompt += `${intro}\nComplete this complex task using the provided paragraphs:\n`;
                prompt += `**Task:** ${input.specific_task}\n`;
