...
export const searchContextPrompt: PromptTemplate = {
    create: (contextItems: QueryContextItem[], query?: string) => {
        ...
        return renderPrompt(
            {
                introduction: `Analyze texts from various blog posts based on the user's input. The user has ${hasQuery ? `searched for "${query}"`: ""}${hasFilters? `${hasQuery? ". and has": ""}expressed interest in following topics & filters: `+filters : ""}`,
                instructions: [
                    ...
                    hasQuery? `Step 2: Determine if user query:("${query}") is a question or not\n`: "",
                ...
                output: `output the result in the following JSON format: 
                        \`\`\`json{
                                ...
                                ${hasQuery? `"concepts": ["concept1", "concept2", "concept3"]`: ""}
                            }
                        \`\`\`
                `
            })
