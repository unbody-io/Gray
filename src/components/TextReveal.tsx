...
function useContainerSizes(ref: React.RefObject<any>): {
    lineHeight: number;
    lineWidth: number;
} {
        ...
        <div ref={ref} className='relative'>
            <div
                className={`transition-all ${
                    render || withDefaultSkeleton ? 'opacity-1' : 'opacity-0'
                }`}
            >
                {children}
            </div>
                    ...
                    {new Array(lineCount || 4).fill(0).map((_, index) => (
                        <Skeleton
                            ...
                            style={{
                                ...
                                width: revealedLines.includes(index)
                                    ? `0%`
                                    : `${((widths[index] + 10) / lineWidth) * 100}%`,
                                right: `${
                                    100 - ((widths[index] + 10) / lineWidth) * 100
                                }%`,
                                transition: `all ${calculateTransitionSpeed(
                                    lineWidth
                                )}ms`,
