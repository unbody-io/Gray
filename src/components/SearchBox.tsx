...
import {ChatIcon, MenuIcon, SearchIcon} from "@/components/icons";
...
import {ESearchMode, KeywordColor} from "@/types/ui.types";
...
const SearchInput = ({placeHolder, label, mode, onValueChange, onClear, ...rest}: SearchInputProps) => {
        ...
        <Input
            ...
            startContent={
                mode === ESearchMode.Read
                    ? <ChatIcon
                        className="text-black/50 fill-black/30 mb-0.5 dark:text-white/90 text-slate-400 pointer-events-none flex-shrink-0"/>
                    :
                    <SearchIcon
...
export const SearchBox = (props: SearchBoxProps) => {
    ...
    const [internalQuery, setInternalQuery] = React.useState<string | undefined>(query);
            ...
            <div className={"flex w-full flex-col relative"}>
                <Tabs aria-label="Options"
                      ...
                      className={clsx(
                          ...
                          `opacity-${inputFocused ? 100 : 50} hover:opacity-100 transition-opacity duration-300`,
                      )}
                >
                ...
                <form onSubmit={onSubmit} autoComplete={"off"}>
                    <SearchInput placeHolder={SearchInputPlaceholder[mode]}
                                 mode={mode}
