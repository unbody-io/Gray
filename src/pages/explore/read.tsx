        ...
        <p className={""}>
            {
                sentencesInBlock.map((sentence, i) => {
                    ...
                    const highestSimilarity = Math.max(
                        ...sentencesInMostSimilarBlock.map(s => getSimilarityScore(sentence, s))
                    )
                            ...
                            content={
                                <div className="px-1 py-2 text-gray-500">
                                    ...
                                    <div className={"text-gray-500 mt-2"}>
                                        ...
                                        <br/>
                                        <div>
                                            <Link href={`/posts/${mostSimilarBlock.document[0].slug}#p-${mostSimilarBlock.order}`}
    ...
    const generatePage = async () => {
        ...
        try {
            const {data}: {data: ReadPageResponse|null} = await fetch(
                buildQueryUrl(`/api/read/page`, queryParams, ["entities", "topics", "keywords", "query"])||"/api/read/page",
                {
                    headers: {
        ...
        <DefaultLayout>
            <div className={"flex flex-col gap-4 mt-6"}>
                <p className={"text-sm text-gray-500"}>
                    In this page you can read your own narrative of our blog. Start by either
                    entering a search prompt or selecting a topic of interest, or both.
                </p>
            ...
            <article className={"prose mt-4 prose-img:rounded-xl prose-a:text-blue-600 max-w-screen-md pb-12"}>
                {
                    pages.map((page, i) => (
                        <>
                            <Page data={page}
                                  loading={!mounted && pagesLoading}
                                  key={i}
                            />
                            <Divider/>
                        </>
